...
class Snekgame(gym.Env):
    ...
    def __init__(self, max_board_size=7):
        ...
        self.observation_space = spaces.Box(shape=((centerpoint * centerpoint) + num_health_flags + \
    ...
    def step(self, action):
        ...
        if self.gameOverFlag and self.winFlag:
            reward = self.winReward + 2 * int(self.JsonServerData["turn"])
            self.diag_wl += 1
            self.wins += 1
        
        if self.gameOverFlag and self.winFlag == False:
            reward = self.dieReward #+ 0.25 * int(self.JsonServerData["turn"])
            self.loses += 1
    ...
    def reset(self):
        ...
        self.winFlag = False
        self.gameOverFlag = False
        self.gameLengthAvg = (self.gameLengthAvg*4/5) + (self.lastGameLength/5)
        self.averageFoodEaten = (self.averageFoodEaten*4/5) + (self.diag_food/5)
        print("")
    ...
    def findObservation(self, data):
        ...
        for enemy_snake in board["snakes"]:
            ...
            enemy_head_val = self.headZeroHP
        ...
        observation[0: viewsize * viewsize] = np.ndarray.flatten(board_state)
